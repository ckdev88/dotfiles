...
# TypeScript React syntax - standalone version

if exists("b:current_syntax")
  finish
endif

var main_syntax = 'typescriptreact'
...
syntax region tsxTag
      ...
      \ end=+\(/>\)\@=+
      \ contained
      \ contains=tsxTagName,tsxIntrinsicTagName,tsxAttrib,tsxEscJs,tsxTemplateAttr,
                \tsxCloseString,@tsxComment

syntax match tsxTag /<>/ contained

# <tag></tag>
# and self close tag <tag/>
syntax region tsxRegion
      ...
      \ fold
      \ contains=tsxRegion,tsxCloseString,tsxCloseTag,tsxTag,tsxCommentInvalid,tsxFragment,tsxEscJs,tsxTemplateAttr,@Spell
      \ keepend
      \ extend
...
syntax match tsxCloseString +/>+ contained

# Template literals in JSX attributes - specific handling
syntax region tsxTemplateAttr
      \ start=+`+
      \ end=+`+
      \ contained
...
syntax match tsxEqual +=+ display contained
  \ nextgroup=tsxString,tsxTemplateAttr skipwhite

# <tag id="sample">
syntax region tsxString contained start=+["']+ end=+["']+ contains=tsxEntity,@Spell display

# Improved JSX attribute value handling
syntax region tsxEscJs
    \ contained
    \ contains=@typescriptValue,typescriptObjectSpread,typescriptTemplate,typescriptTemplateSubstitution
    \ matchgroup=typescriptBraces
    \ start=+{+
    \ end=+}+
    \ extend

# Load base TypeScript syntax
...
highlight def link tsxTemplateAttr typescriptTemplate

b:current_syntax = "typescriptreact"
